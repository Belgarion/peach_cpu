...
void cl_md2_transform(CUDA_MD2_CTX *ctx, BYTE data[])
{
	int j,k,t;
...
void cl_md2_update(CUDA_MD2_CTX *ctx, /*const*/BYTE data[], size_t len)
{
	size_t i;
...
void cl_md2_final(CUDA_MD2_CTX *ctx, BYTE hash[])
{
	...
	cl_md2_transform(ctx, ctx->data);
	cl_md2_transform(ctx, ctx->checksum);
