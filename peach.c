...
void cl_gen_tile(uint32_t index, __global uint8_t *g_map, uint8_t debug, __global uint8_t *c_phash)
{
   ...
   for(i = 0, j = HASHLEN; j < TILE_LENGTH; i += HASHLEN, j += HASHLEN) { /* For each tile row */
	   ...
	   /*printf("tile %d (algo_type: %d): ", i, nighthash.algo_type);
	   printf32("", (tilep+i));*/
	   ...
	   /*printf("tile %d (algo_type: %d): ", j, nighthash.algo_type);
	   printf32("", (tilep+j));*/
   }
}


__kernel void cl_build_map(__global uint8_t *g_map, __global uint8_t *c_phash, uint32_t start_index, uint32_t thread)
{
   //uint32_t thread = get_global_id(0) + start_index;
   thread = thread + start_index;
   if (thread < MAP) {
